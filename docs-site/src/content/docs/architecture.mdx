...
| `backup_service.py` | WAL-safe snapshots, immutable read-only vault inspection, queued restore and safety copy | Live counts remain WAL-aware; closed vault artifacts reject non-empty sibling WAL files, and an unverified or failed staged copy never replaces the live file. |
...
└── services/               Lifecycle, valuation, review, backup, DCA, cache, analytics, signals, updates
...
├── js/review-orbit.js      Review workspace, reports, compare, thesis, and vault UI
