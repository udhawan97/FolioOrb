...
| `backup_service.py` | WAL-safe snapshots, integrity verification, vault inventory, queued restore and safety copy | Manual restore happens before SQLAlchemy opens the database; an unverified or failed staged copy never replaces the live file. |
